...
function point_inside_triangle(s, a, b, c) {
    /* Determine whether the given point (s) is within the triangle formed by points A, B and 
C */
...
function calculate_fps() {
	...
	draw_font( 'Frames Per Second: ' + (fps).toFixed(1), 50, 50, '10pt Helvetica', 
'black' ); */
    ...
    draw_font('Frames Per Second: ' + (1000 / frame_time).toFixed(0), area_width - 200,
area_height - 30, '10pt Helvetica', 'black');
}
...
function draw_user() {
    context.drawImage(smiley, user.x - (smiley.width / 4), user.y - (smiley.height / 4),
smiley.width / 2, smiley.height / 2);
}
...
function update_spheres() {
    ...
    for (var i = spheres.length; i--;) {
        ...
        if (spheres[i].x > area_width + 1 || spheres[i].x < -1 || spheres[i].y > area_height + 1
|| spheres[i].y < -1) {
            spheres[i].vx *= -1;
            spheres[i].vy *= -1;
            continue;
...
var is_mobile =
{
    ...
    any: function () {
        return (is_mobile.android() || is_mobile.black_berry() || is_mobile.ios() ||
is_mobile.opera_mini() || is_mobile.windows());
    }
};
...
function on_load() {
    ...
    var dimension = [document.documentElement.clientWidth,
